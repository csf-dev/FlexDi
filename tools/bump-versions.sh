...
function bump_nuspec_version()
{
  ...
  find "$SOLUTION_ROOT" \
    \! -path "*/.git/*" \
    -name "*.nuspec" \
    -exec sed -ri "s/<dependency id=\"CSF\\.FlexDi\" version=\"[^\"]+\" \\/>/<dependency id=\"CSF.FlexDi\" version=\"${NEW_VERSION}\" \\/>/g" '{}' \;
}

function bump_solution_version()
{
  sed -ri "s/version *= *.+/version = ${NEW_VERSION}/g" "$SOLUTION_ROOT/CSF.FlexDi.sln"
