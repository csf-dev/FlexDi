...
namespace CSF.FlexDi.Tests.Registration
{
    ...
    public class StackOfRegistriesRegistrationProviderTests
    {
        ...
        {
            // Arrange
            Mock.Get(providers[0]).Setup(x => x.HasRegistration(request)).Returns(false);
            Mock.Get(providers[1]).Setup(x => x.HasRegistration(request)).Returns(true);
            Mock.Get(providers[2]).Setup(x => x.HasRegistration(request)).Returns(false);
                                                                                               ...
                                                                                               ResolutionRequest request)
        {
            // Arrange
            Mock.Get(providers[0]).Setup(x => x.HasRegistration(request)).Returns(false);
            Mock.Get(providers[1]).Setup(x => x.HasRegistration(request)).Returns(false);
            Mock.Get(providers[2]).Setup(x => x.HasRegistration(request)).Returns(false);
            var sut = new StackOfRegistriesRegistrationProvider(providers);
                                                                                 ...
                                                                                 IServiceRegistration registration)
        {
            ...
            Mock.Get(providers[1]).Setup(x => x.HasRegistration(request)).Returns(true);
            Mock.Get(providers[1]).Setup(x => x.Get(request)).Returns(registration);
            Mock.Get(providers[2]).Setup(x => x.HasRegistration(request)).Returns(true);
